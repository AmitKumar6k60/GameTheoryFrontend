...
const BookingPage = () => {
  ...
  const fetchCentres = async () => {
    try {
      const response = await fetch('http://localhost:5001/api/centres');
  ...
  const fetchSports = async (centreId) => {
    try {
      const response = await fetch(`http://localhost:5001/api/sports_with_centreId/${centreId}`);
      const data = await response.json();
  ...
  const fetchResources = async (centreId, sportId) => {
    try {
      const response = await fetch(`http://localhost:5001/api/resources/${centreId}/${sportId}`);
      const data = await response.json();
      if (Array.isArray(data)) {
        setResources(data);
      } else {
